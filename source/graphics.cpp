...
namespace mc
{
    ...
    void initPipelines( mc::AppContext* app )
    {
        ...
        // Set up shader used to perform masked cut
        {
            wgpu::ShaderModuleWGSLDescriptor cutShaderCodeDesc;
            cutShaderCodeDesc.code = b::embed<"./resources/shaders/maskmultiply.wgsl">().data();

            wgpu::ShaderModuleDescriptor cutShaderDesc;
            cutShaderDesc.nextInChain = &cutShaderCodeDesc;
            ...
            std::array<wgpu::ColorTargetState, 2> cutRenderTargets;

            cutRenderTargets[0].format    = wgpu::TextureFormat::RGBA8Unorm;
            cutRenderTargets[0].blend     = &blendState;
            cutRenderTargets[0].writeMask = wgpu::ColorWriteMask::All;

            cutRenderTargets[1].format    = wgpu::TextureFormat::RGBA8Unorm;
            cutRenderTargets[1].blend     = &blendState;
            cutRenderTargets[1].writeMask = wgpu::ColorWriteMask::All;

            wgpu::FragmentState cutFragmentState;
            cutFragmentState.module        = cutShaderModule;
            cutFragmentState.entryPoint    = "fs_cut";
            cutFragmentState.constantCount = 0;
            cutFragmentState.targetCount   = cutRenderTargets.size();
            cutFragmentState.targets       = cutRenderTargets.data();

            wgpu::VertexState cutVertexState;
            cutVertexState.module        = cutShaderModule;
            cutVertexState.entryPoint    = "vs_cut";
            cutVertexState.bufferCount   = 0;
            cutVertexState.constantCount = 0;

            std::array<wgpu::BindGroupLayout, 2> cutBindGroupLayouts = { textureGroupLayout, textureGroupLayout };

            wgpu::PipelineLayoutDescriptor cutPipelineLayoutDesc;
            cutPipelineLayoutDesc.bindGroupLayoutCount = static_cast<uint32_t>( cutBindGroupLayouts.size() );
            cutPipelineLayoutDesc.bindGroupLayouts     = cutBindGroupLayouts.data();

            wgpu::PipelineLayout postPipelineLayout = app->device.CreatePipelineLayout( &cutPipelineLayoutDesc );

            wgpu::RenderPipelineDescriptor cutMaskPipelineDesc;
            cutMaskPipelineDesc.label                              = "Cut Mask";
            cutMaskPipelineDesc.vertex                             = cutVertexState;
            cutMaskPipelineDesc.fragment                           = &cutFragmentState;
            cutMaskPipelineDesc.layout                             = postPipelineLayout;
            cutMaskPipelineDesc.primitive.topology                 = wgpu::PrimitiveTopology::TriangleList;
            cutMaskPipelineDesc.primitive.stripIndexFormat         = wgpu::IndexFormat::Undefined;
            cutMaskPipelineDesc.primitive.frontFace                = wgpu::FrontFace::CCW;
            cutMaskPipelineDesc.multisample.count                  = 1;
            cutMaskPipelineDesc.multisample.mask                   = ~0u;
            cutMaskPipelineDesc.multisample.alphaToCoverageEnabled = false;

            app->cutMaskPipeline = app->device.CreateRenderPipeline( &cutMaskPipelineDesc );
        }
